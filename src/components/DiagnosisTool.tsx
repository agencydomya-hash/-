...
export default function DiagnosisTool({ onDiagnosisComplete, onSelectBookingWithDiagnosis }: DiagnosisToolProps) {
  ...
  return (
    <div className="bg-[#f8fafc] py-16 border-y border-gray-200" id="diagnosis-tool">
        ...
        <AnimatePresence mode="wait">
          ...
          {!loading && !result && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="bg-white p-6 sm:p-10 rounded-2xl shadow-xl border border-gray-100 text-right"
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-bold text-gray-700">الاسم الكريم</label>
                    <input
                      type="text"
                      name="name"
                      required
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF8C00] focus:border-[#FF8C00] transition outline-none text-gray-800"
                    />
                  </div>
                ...
                {/* Specialty Selection */}
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">تخصصك الطبي</label>
                    ...
                    {COMMON_SPECIALTIES.map((spec, idx) => (
                      <button
                        ...
                        className={`px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition border ${
                          form.specialty === spec
                            ? 'bg-[#1A2B5B] text-white border-[#1A2B5B] shadow-md shadow-blue-950/20'
                            : 'bg-white text-gray-600 border-gray-200 hover:border-[#1A2B5B]/30'
                        }`}
                      >
                        {spec}
                      </button>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full mt-2 px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF8C00] focus:border-[#FF8C00] transition outline-none text-gray-800"
                  />
                </div>

                {/* Target Audience */}
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">مين هو جمهورك أو مريضك المستهدف بالتحديد؟</label>
                  <input
                    type="text"
                    name="targetAudience"
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">إيه أكتر تحدي أو عَرَض بتعاني منه في السوشيال ميديا؟</label>
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    rows={2}
                    className="w-full mt-2 px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#FF8C00] focus:border-[#FF8C00] transition outline-none text-gray-800 text-sm"
                  />
                </div>
                ...
                <div className="pt-4 flex justify-start">
                  <button
                    type="submit"
                    className="w-full sm:w-auto px-10 py-4 bg-[#FF8C00] hover:bg-orange-600 text-white font-bold rounded-xl transition duration-300 shadow-lg shadow-orange-500/20 flex items-center justify-center gap-3 text-lg"
                    id="submit-diagnosis"
                  >
          ...
          {loading && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="bg-white p-12 rounded-2xl shadow-xl border border-gray-100 text-center flex flex-col items-center justify-center min-h-[400px]"
            ...
            >
              <div className="relative mb-6">
                ...
                <Stethoscope className="w-8 h-8 text-[#1A2B5B] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-bounce" />
              </div>
              ...
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="text-lg font-bold text-[#1A2B5B] max-w-md"
              >
          ...
          {result && (
            ...
            >
              <div
                className="bg-white rounded-2xl shadow-2xl border-4 border-[#1A2B5B]/10 p-6 sm:p-10 relative overflow-hidden print:shadow-none print:border-none"
                id="printable-prescription-sheet"
              >
                {/* Paper header strip */}
                <div className="absolute top-0 inset-x-0 h-3 bg-gradient-to-r from-[#1A2B5B] via-[#FF8C00] to-[#1A2B5B]"></div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b-2 border-gray-100 pb-6 mb-6 gap-4 text-right">
                  <div>
                    <div className="flex items-center gap-2 mb-1 justify-end">
                      <span className="px-2 py-0.5 rounded bg-orange-100 text-[#FF8C00] text-[10px] font-bold">معتمد طبيّاً</span>
                      <h3 className="text-xl font-bold text-[#1A2B5B]">عيادة البراند الطبي من دومايا</h3>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-gray-50/80 p-4 rounded-xl border border-gray-100 text-right mb-6">
                  <div>
                    ...
                    <span className="text-sm font-bold text-[#1A2B5B]">د. {result.patientName}</span>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-7 space-y-6 text-right">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-sm font-bold text-[#1A2B5B] border-r-4 border-red-500 pr-2 flex items-center gap-1.5 justify-start">
                    ...
                    {/* Rx Action steps */}
                    <div className="space-y-3">
                      <h4 className="text-sm font-bold text-[#1A2B5B] border-r-4 border-[#FF8C00] pr-2 flex items-center gap-1.5 justify-start">
                  ...
                  <div className="lg:col-span-5 space-y-6 text-right">
                    {/* Content Plan */}
                    <div className="bg-[#1A2B5B]/5 p-5 rounded-2xl border border-[#1A2B5B]/10 space-y-4">
                      <h4 className="text-sm font-bold text-[#1A2B5B] border-r-4 border-[#1A2B5B] pr-2">
                        بروتوكول المحتوى المقترح (Reels)
                      </h4>
                      <div className="space-y-2">
                        <span className="text-[10px] text-gray-500 block">العنوان والمحور العام للمحتوى:</span>
                        <div className="text-sm font-bold text-[#1A2B5B] bg-white p-3 rounded-lg border border-gray-100">
                        ...
                        <div className="space-y-2">
                          {result.contentPlan.topics.map((topic, idx) => (
                            <div key={idx} className="bg-white p-2.5 rounded-lg border border-gray-100 text-xs font-semibold text-gray-700 flex items-center gap-2">
                              <span className="w-5 h-5 rounded-full bg-[#1A2B5B]/10 text-[#1A2B5B] flex items-center justify-center font-mono font-bold flex-shrink-0 text-[10px]">
                                {idx + 1}
                              </span>
                              <span>{topic}</span>
                            </div>
                ...
                <div className="border-t-2 border-dashed border-gray-100 mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center text-center gap-4 text-xs text-gray-400">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-[#1A2B5B]">Domya Marketing Agency - 2026</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 bg-[#FF8C00] hover:bg-orange-600 text-white font-bold rounded-xl transition duration-300 flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20 text-sm animate-bounce"
                  id="diag-to-booking"
                >
