...
export default function Testimonials() {
  return (
    <section className="py-20 bg-[#f8fafc] border-b border-gray-200" id="testimonials-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center mb-16 space-y-3">
          <span className="text-[#FF8C00] font-bold uppercase tracking-wider text-sm block">قصص نجاح حقيقية</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {TESTIMONIALS.map((test, idx) => (
            <motion.div
              ...
              className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-100 flex flex-col justify-between relative group hover:border-[#FF8C00]/30 transition duration-300"
            >
              {/* Top Quote Icon decoration */}
              <div className="absolute top-6 left-6 text-gray-100 select-none pointer-events-none group-hover:text-orange-500/10 transition duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-1 justify-start">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 text-[#FF8C00] fill-current" />
                  ))}
                </div>

                {/* Quote text */}
                <p className="text-gray-600 text-xs sm:text-sm leading-relaxed text-right font-medium italic">
                  "{test.quote}"
                </p>
              </div>

              {/* Doctor Profile Banner */}
              <div className="mt-8 pt-6 border-t border-gray-100 flex items-center gap-3.5 text-right">
                <div className="relative">
                  <img
                    src={test.avatar}
                    alt={test.name}
                    className="w-12 h-12 rounded-full object-cover border-2 border-[#1A2B5B]"
                    referrerPolicy="no-referrer"
                  />
                  <div className="w-4 h-4 bg-orange-500 rounded-full absolute bottom-0 right-0 border-2 border-white flex items-center justify-center text-[8px] text-white">
                    ✓
                  </div>
                </div>
                
                <div>
                  <h4 className="font-bold text-[#1A2B5B] text-sm sm:text-base">{test.name}</h4>
                  <p className="text-[10px] sm:text-xs text-gray-500 font-semibold">{test.specialty}</p>
                  
                  {/* Result Badge */}
                  <div className="mt-2 inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 text-[10px] font-bold">
        ...
        <div className="mt-16 bg-gradient-to-r from-[#1A2B5B] to-[#121f42] p-8 rounded-3xl text-center text-white flex flex-col sm:flex-row justify-between items-center gap-6">
